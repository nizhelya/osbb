...
public function delOplata(stdClass $params)
	    ...
	    public function getRaspechatka(stdClass $params)
	{
		...
		switch ($this->what) {
			...
			case "AktLgotaUtsznAll":
			  switch ($this->usluga_id) {
				...
				break;		
				case "6":
				      $this->sql='CALL OSBB.AktLgotaUtsznAll_gas('.$this->osmd_id.',"'.$this->data.'",@success,@content)';
			...
			case "AktSubsUtsznAll":
				...
				break;		
				case "6":
				      $this->sql='CALL OSBB.AktSubsidiaUtsznAll_gas('.$this->osmd_id.',"'.$this->data.'",@success,@content)';
			...
			case "Forma3LgotaUtszn":
			  switch ($this->usluga_id) {
				...
				break;		
				case "6":
				      $this->sql='CALL OSBB.Forma3LgotaUtszn_gas('.$this->osmd_id.',"'.$this->data.'",@success,@content)';
			...
			case "AktSubsidiaOsmd":
				      $this->sql='CALL OSBB.AktSubsidiaAll('.$this->osmd_id.',@success,@content)';
			   // print_r($this->sql); 
			break;
			case "ReestrLgotaUtsznAll":
			  switch ($this->usluga_id) {
				...
				break;		
				case "6":
				      $this->sql='CALL OSBB.ReestrLgotaUtsznAll_gas('.$this->osmd_id.',"'.$this->data.'",@success,@content)';
			...
			case "ReestrSubsidiaUtsznAll":
				...
				break;
				case "6":
				      $this->sql='CALL OSBB.ReestrSubsidiaUtsznAll_gas('.$this->osmd_id.',"'.$this->data.'",@success,@content)';
				break;
				}
			break;
			case "PrintRaspechatka":
				      $this->sql='CALL OSBB.raspechatkaApp('.$this->osmd_id.',@success,@content)';
			  //  print_r($this->sql); 
			break;
			}
		...
		//print_r($this->results['email']);
			...
			$mail->AddAddress( $this->email); // е-мэил кому отправлять
			$mail->AddReplyTo('yis@yuzhny.com','Info'); // е-мэил того кому прейдет ответ на ваше письмо
			$mail->WordWrap = 350;// set word wrap
			//$mail->AddAttachment($filename);         // add attachments
			$mail->IsHTML(true);// отправить в html формате
			$mail->CharSet    = 'utf-8';
			$mail->Subject  =  'Распечатка'; // тема письма
			$mail->Body     =  $this->results['content'] ; // тело письма в html формате
