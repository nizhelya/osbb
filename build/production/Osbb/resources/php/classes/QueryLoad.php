...
class QueryLoad
{
	...
	public function getResults(stdClass $params)
	{
		...
		break;
		case "getOsmd":
				$this->sql='SELECT t1.`osmd_id`,t1.`osmd` FROM YISGRAND.OSMD as t1 WHERE t1.`osmd_id`  in('.$this->osmd_id.')' ;
					 //  print($_sql);
		break;
		case "osmd":
	...
	case "Tarif":
		...
		break;
		case "TarifFromHouse":
				$this->sql='SELECT * FROM OSBB.TARIF as t1 WHERE t1.`house_id` ='.$this->house_id.' and  t1.`data` = CONCAT(EXTRACT(YEAR_MONTH FROM CURDATE()),"01")';
					   // print($_sql); 
		break;
		case "TarifFromHouses":
				$this->sql='SELECT * FROM OSBB.TARIF as t1 WHERE t1.`house_id` in( '.$this->house_id.') and  t1.`data` = CONCAT(EXTRACT(YEAR_MONTH FROM CURDATE()),"01")' ;
					//  print($this->sql); 
		break;
	...
	case "AddressFromHousesTarif":
					...
					$this->sql='SELECT * FROM '.$this->base.'.APPARTMENT as t1 WHERE t1.`house_id` in('.$this->house_id.')';
			   // print($_sql);
