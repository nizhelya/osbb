	...
	$array = (array) $params;
		...
		switch ($this->what) {
			...
			switch ($this->usluga_id) {
				case "1":
				$this->sql='SELECT *  FROM OSBB.LG_KVARTPLATA as t1 WHERE t1.`osmd_id` = '.$this->osmd_id.' AND  t1.`data` =CONCAT(EXTRACT(YEAR_MONTH FROM "'.$this->data.'"),"01") ';
				break;				
				case "2":
				$this->sql='SELECT *  FROM OSBB.LG_TEPLO as t1 WHERE  t1.`osmd_id` = '.$this->osmd_id.' AND t1.`data` =CONCAT(EXTRACT(YEAR_MONTH FROM "'.$this->data.'"),"01") ';
				break;
				case "3":
				     $this->sql='SELECT *  FROM OSBB.LG_VODA as t1 WHERE  t1.`osmd_id` = '.$this->osmd_id.' AND t1.`data` =CONCAT(EXTRACT(YEAR_MONTH FROM "'.$this->data.'"),"01")';
				break;
				case "4":
				     $this->sql='SELECT *  FROM OSBB.LG_ENERGY as t1 WHERE  t1.`osmd_id` = '.$this->osmd_id.' AND t1.`data` =CONCAT(EXTRACT(YEAR_MONTH FROM "'.$this->data.'"),"01")';
				break;
				case "5":
				     $this->sql='SELECT *  FROM OSBB.LG_TBO as t1 WHERE  t1.`osmd_id` = '.$this->osmd_id.' AND t1.`data` =CONCAT(EXTRACT(YEAR_MONTH FROM "'.$this->data.'"),"01")';
				break;
				case "6":
				     $this->sql='SELECT  * FROM OSBB.LG_TEPLO as t1 WHERE  t1.`osmd_id` = '.$this->osmd_id.' AND t1.`data` =CONCAT(EXTRACT(YEAR_MONTH FROM "'.$this->data.'"),"01") ';
				break;
			...
			case "getPrivatBank"://применяется
				  $this->sql='SELECT *  FROM OSBB.PRIVATBANK_OPLATA';
			break;
			...
			case "getUsluga"://применяется
				  $this->sql='SELECT *  FROM OSBB.SPR_USLUGA ORDER BY `usluga_id` ';
			...
			case "getSubsidiaVozvrat":		
			      $this->sql='SELECT  * FROM OSBB.SUBSIDV_TEMP as t1 WHERE 1 ORDER BY t1.`data` DESC ';

			break;
			case "getControlSubsidiaUtszn":
			      switch ($this->usluga_id) {
			      case "1":
				     $this->sql='SELECT  t1.* FROM OSBB.SKV as t1  WHERE   t1.`data` =CONCAT(EXTRACT(YEAR_MONTH FROM "'.$this->data.'"),"01") and ' 
			      .' ((t1.dolg != t1.zadol + t1.perer - t1.oplata  - t1.doplata)  or (t1.`nachisleno` != t1.`perer`) OR (t1.fin != t1.st- t1.doplata))';
				break;
				
				case "2":
				$this->sql='SELECT  t1.* FROM OSBB.SOT as t1  WHERE   t1.`data` =CONCAT(EXTRACT(YEAR_MONTH FROM "'.$this->data.'"),"01") and ' 
			      .' ((t1.dolg != t1.zadol + t1.perer - t1.oplata  - t1.doplata)  or (t1.`nachisleno` != t1.`perer`) OR (t1.fin != t1.st- t1.doplata))';
				break;
				case "3":
				$this->sql='SELECT  t1.* FROM OSBB.SXV as t1  WHERE   t1.`data` =CONCAT(EXTRACT(YEAR_MONTH FROM "'.$this->data.'"),"01") and ' 
			      .' ((t1.dolg != t1.zadol + t1.perer - t1.oplata  - t1.doplata)  or (t1.`nachisleno` != t1.`perer`) OR (t1.fin != t1.st- t1.doplata))';
				break;
				case "4":
				$this->sql='SELECT  t1.* FROM OSBB.SEN as t1  WHERE   t1.`data` =CONCAT(EXTRACT(YEAR_MONTH FROM "'.$this->data.'"),"01") and ' 
			      .' ((t1.dolg != t1.zadol + t1.perer - t1.oplata  - t1.doplata)  or (t1.`nachisleno` != t1.`perer`) OR (t1.fin != t1.st- t1.doplata))';
				break;
				case "5":
				$this->sql='SELECT  t1.* FROM OSBB.STB as t1  WHERE   t1.`data` =CONCAT(EXTRACT(YEAR_MONTH FROM "'.$this->data.'"),"01") and ' 
			      .' ((t1.dolg != t1.zadol + t1.perer - t1.oplata  - t1.doplata)  or (t1.`nachisleno` != t1.`perer`) OR (t1.fin != t1.st- t1.doplata))';
				break;
				case "6":
				$this->sql='SELECT  t1.* FROM OSBB.SPT as t1  WHERE   t1.`data` =CONCAT(EXTRACT(YEAR_MONTH FROM "'.$this->data.'"),"01") and ' 
			      .' ((t1.dolg != t1.zadol + t1.perer - t1.oplata  - t1.doplata)  or (t1.`nachisleno` != t1.`perer`) OR (t1.fin != t1.st- t1.doplata))';
				break;
				}		    
			...
			break;
			case "subsidia"://применяется
				  $this->sql='SELECT *  FROM OSBB.SUBSID';
			break;
			case "getSubsidia"://применяется
			switch ($this->usluga_id) {
				case "1":
				$this->sql='SELECT *  FROM OSBB.SUBS_KVARTPLATA as t1 WHERE t1.`osmd_id` = '.$this->osmd_id.' AND  t1.`data` =CONCAT(EXTRACT(YEAR_MONTH FROM "'.$this->data.'"),"01") ';
				break;				
				case "2":
				$this->sql='SELECT *  FROM OSBB.SUBS_TEPLO as t1 WHERE  t1.`osmd_id` = '.$this->osmd_id.' AND t1.`data` =CONCAT(EXTRACT(YEAR_MONTH FROM "'.$this->data.'"),"01") ';
				break;
				case "3":
				     $this->sql='SELECT *  FROM OSBB.SUBS_VODA as t1 WHERE  t1.`osmd_id` = '.$this->osmd_id.' AND t1.`data` =CONCAT(EXTRACT(YEAR_MONTH FROM "'.$this->data.'"),"01")';
				break;
				case "4":
				     $this->sql='SELECT *  FROM OSBB.SUBS_ENERGY as t1 WHERE  t1.`osmd_id` = '.$this->osmd_id.' AND t1.`data` =CONCAT(EXTRACT(YEAR_MONTH FROM "'.$this->data.'"),"01")';
				break;
				case "5":
				     $this->sql='SELECT *  FROM OSBB.SUBS_TBO as t1 WHERE  t1.`osmd_id` = '.$this->osmd_id.' AND t1.`data` =CONCAT(EXTRACT(YEAR_MONTH FROM "'.$this->data.'"),"01")';
				break;
				case "6":
				     $this->sql='SELECT  * FROM OSBB.SUBS_TEPLO as t1 WHERE  t1.`osmd_id` = '.$this->osmd_id.' AND t1.`data` =CONCAT(EXTRACT(YEAR_MONTH FROM "'.$this->data.'"),"01") ';
				break;
			...
			case "getSubsidiaUtszn":
			      switch ($this->usluga_id) {
			      case "1":
				     $this->sql='SELECT  * FROM OSBB.SKV as t1 WHERE t1.`osmd_id` = '.$this->osmd_id.' AND  t1.`data` =CONCAT(EXTRACT(YEAR_MONTH FROM "'.$this->data.'"),"01") ORDER BY t1.`address_id`';

				break;
				
				case "2":
				     $this->sql='SELECT  * FROM OSBB.SOT as t1 WHERE  t1.`osmd_id` = '.$this->osmd_id.' AND t1.`data` =CONCAT(EXTRACT(YEAR_MONTH FROM "'.$this->data.'"),"01") ORDER BY t1.`address_id`';

				break;
				case "3":
				     $this->sql='SELECT  * FROM OSBB.SXV as t1 WHERE  t1.`osmd_id` = '.$this->osmd_id.' AND t1.`data` =CONCAT(EXTRACT(YEAR_MONTH FROM "'.$this->data.'"),"01") ORDER BY t1.`address_id`';

				break;
				case "4":
				     $this->sql='SELECT  * FROM OSBB.SEN as t1 WHERE  t1.`osmd_id` = '.$this->osmd_id.' AND t1.`data` =CONCAT(EXTRACT(YEAR_MONTH FROM "'.$this->data.'"),"01") ORDER BY t1.`address_id`';

				break;
				case "5":
				     $this->sql='SELECT  * FROM OSBB.STB as t1 WHERE  t1.`osmd_id` = '.$this->osmd_id.' AND t1.`data` =CONCAT(EXTRACT(YEAR_MONTH FROM "'.$this->data.'"),"01") ORDER BY t1.`address_id`';

				break;
				case "6":
				     $this->sql='SELECT  * FROM OSBB.SPT as t1 WHERE  t1.`osmd_id` = '.$this->osmd_id.' AND t1.`data` =CONCAT(EXTRACT(YEAR_MONTH FROM "'.$this->data.'"),"01") ORDER BY t1.`address_id`';
			...
			case "getLgotaUtszn":		
			      switch ($this->usluga_id) {
			      case "1":
				     $this->sql='SELECT  * FROM OSBB.BKV as t1 WHERE t1.`osmd_id` = '.$this->osmd_id.' AND  t1.`data` =CONCAT(EXTRACT(YEAR_MONTH FROM "'.$this->data.'"),"01") ORDER BY t1.`address_id`';

				break;
				
				case "2":
				     $this->sql='SELECT  * FROM OSBB.BOT as t1 WHERE t1.`osmd_id` = '.$this->osmd_id.' AND  t1.`data` =CONCAT(EXTRACT(YEAR_MONTH FROM "'.$this->data.'"),"01") ORDER BY t1.`address_id`';

				break;
				case "3":
				     $this->sql='SELECT  * FROM OSBB.BXV as t1 WHERE t1.`osmd_id` = '.$this->osmd_id.' AND  t1.`data` =CONCAT(EXTRACT(YEAR_MONTH FROM "'.$this->data.'"),"01") ORDER BY t1.`address_id`';

				break;
				case "4":
				     $this->sql='SELECT  * FROM OSBB.BEN as t1 WHERE t1.`osmd_id` = '.$this->osmd_id.' AND  t1.`data` =CONCAT(EXTRACT(YEAR_MONTH FROM "'.$this->data.'"),"01") ORDER BY t1.`address_id`';

				break;
				case "5":
				     $this->sql='SELECT  * FROM OSBB.BTB as t1 WHERE t1.`osmd_id` = '.$this->osmd_id.' AND  t1.`data` =CONCAT(EXTRACT(YEAR_MONTH FROM "'.$this->data.'"),"01") ORDER BY t1.`address_id`';

				break;
				case "6":
				     $this->sql='SELECT  * FROM OSBB.BPT as t1 WHERE  t1.`osmd_id` = '.$this->osmd_id.' AND t1.`data` =CONCAT(EXTRACT(YEAR_MONTH FROM "'.$this->data.'"),"01") ORDER BY t1.`address_id`';
			...
			case "getControlLgotaUtszn":		
			      switch ($this->usluga_id) {
			      case "1":
				     $this->sql='SELECT  t1.* FROM OSBB.BKV as t1  WHERE   t1.`data` =CONCAT(EXTRACT(YEAR_MONTH FROM "'.$this->data.'"),"01") and ' 
			      .' ((t1.dolg != t1.zadol + t1.perer - t1.oplata  - t1.doplata)  or (t1.`nachisleno` != t1.`perer`) OR (t1.fin != t1.st- t1.doplata))';

				break;
				
				case "2":
				 $this->sql='SELECT  t1.* FROM OSBB.BOT as t1  WHERE   t1.`data` =CONCAT(EXTRACT(YEAR_MONTH FROM "'.$this->data.'"),"01") and ' 
			      .' ((t1.dolg != t1.zadol + t1.perer - t1.oplata  - t1.doplata)  or (t1.`nachisleno` != t1.`perer`) OR (t1.fin != t1.st- t1.doplata))';
				break;
				case "3":
				$this->sql='SELECT  t1.* FROM OSBB.BXV as t1  WHERE   t1.`data` =CONCAT(EXTRACT(YEAR_MONTH FROM "'.$this->data.'"),"01") and ' 
			      .' ((t1.dolg != t1.zadol + t1.perer - t1.oplata  - t1.doplata)  or (t1.`nachisleno` != t1.`perer`) OR (t1.fin != t1.st- t1.doplata))';
				break;
				case "4":
				     $this->sql='SELECT  t1.* FROM OSBB.BEN as t1  WHERE   t1.`data` =CONCAT(EXTRACT(YEAR_MONTH FROM "'.$this->data.'"),"01") and ' 
			      .' ((t1.dolg != t1.zadol + t1.perer - t1.oplata  - t1.doplata)  or (t1.`nachisleno` != t1.`perer`) OR (t1.fin != t1.st- t1.doplata))';

				break;
				case "5":
				      $this->sql='SELECT  t1.* FROM OSBB.BTB as t1  WHERE   t1.`data` =CONCAT(EXTRACT(YEAR_MONTH FROM "'.$this->data.'"),"01") and ' 
			      .' ((t1.dolg != t1.zadol + t1.perer - t1.oplata  - t1.doplata)  or (t1.`nachisleno` != t1.`perer`) OR (t1.fin != t1.st- t1.doplata))';

				break;
				case "6":
				      $this->sql='SELECT  t1.* FROM OSBB.BPT as t1  WHERE   t1.`data` =CONCAT(EXTRACT(YEAR_MONTH FROM "'.$this->data.'"),"01") and ' 
			      .' ((t1.dolg != t1.zadol + t1.perer - t1.oplata  - t1.doplata)  or (t1.`nachisleno` != t1.`perer`) OR (t1.fin != t1.st- t1.doplata))';
	...
	public function createRecord(stdClass $params) // ================================= CREATE RECORD
	{
		...
		case "getDbfLgota"://применяется
				    ...
				    $sql = 'INSERT INTO '.$this->base.'.DBF_LGOTA(`cdpr`, `idcode`, `fio`, `ppos`, `rs`, `yearin`, `monthin`, `lgcode`, `data1`, `data2`,'
										.'`lgkol`, `lgkat`, `lgprc`, `summ`, `fact`, `tarif`, `flag`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
				    $stmt = $_db->prepare($sql) or die('Connect Error in '.$this->what.' ( ' . $sql . ' ) ' . $_db->connect_error);
				    $stmt->bind_param('issssiiissisidddi',$cdpr, $idcode, $fio, $ppos, $rs, $yearin, $monthin, $lgcode, $data1, $data2,$lgkol, $lgkat, $lgprc, $summ, $fact, $tarif, $flag);
				    $cdpr	=  $params->cdpr;
...
//print($sql);
		...
		switch ($this->what) {
		    ...
		    $id = $params->rec_id;
				$sql = 'DELETE FROM OSBB.PRIVATBANK_OPLATA  WHERE rec_id = ? LIMIT 1';
					  $stmt = $_db->prepare($sql) or die('Connect Error in '.$this->what.' ( ' . $sql . ' ) ' . $_db->connect_error);
					  $stmt->bind_param('i', $id);
					  $stmt->execute();
					  $stmt->close();					 
		...
		switch ($this->what) {
			case "getDbfLgota"://применяется
					    ...
					    .'`lgcode` = ?, `data1` = ?, `data2` = ?,`lgkol` = ?, `lgkat` = ?, `lgprc` = ?, `summ` = ?, `fact` = ?, `tarif` = ?, `flag` = ?  WHERE '.$this->base.'.DBF_LGOTA.`rec_id` = ? ';
					    $stmt = $_db->prepare($sql) or die('Connect Error in '.$this->what.' ( ' . $sql . ' ) ' . $_db->connect_error);
					    $stmt->bind_param('issssiiissisidddii',$cdpr, $idcode, $fio, $ppos, $rs, $yearin, $monthin, $lgcode, $data1, $data2,$lgkol, $lgkat, $lgprc, $summ, $fact, $tarif, $flag,$rec_id);
...
//print($sql);
			...
			case "getSubsidiaUtszn":	
				    ...
				    $sql = 'UPDATE OSBB.SUBSID_UTSZN SET st = ?,subsidia = ?,oplata = ?,doplata = ?,fin = ? WHERE OSBB.SUBSID_UTSZN.rec_id=?';
			...
			case "getPrivatBank":
				  ...
				  $sql = 'UPDATE OSBB.PRIVATBANK_OPLATA SET summa = ?,pr = ? WHERE OSBB.PRIVATBANK_OPLATA.rec_id=? ';
			...
			case "subsidia":
				    ...
				    $sql = 'UPDATE '.$this->base.'.SUBSID SET kvartplata=?,otoplenie=?,podogrev=?,voda=?,stoki=?,tbo=?,rfond=?,energy=?,summa=? WHERE '.$this->base.'.SUBSID.rec_id=? ';

				    $stmt = $_db->prepare($sql) or die('Connect Error in '.$this->what.' ( ' . $sql . ' ) ' . $_db->connect_error);

				    $stmt->bind_param('dddddddddi', $kvartplata,$otoplenie,$podogrev,$voda, $stoki,$tbo, $rfond,$energy,$summa,$rec_id);
