...
//print($this->what);
			...
			{
			case 'utszn':
			$filename = "/tmp/osbb.dbf";
			$this->sql='SELECT * FROM OSBB.DBF_LGOTA as t1 WHERE  t1.`data` =CONCAT(EXTRACT(YEAR_MONTH FROM "'.$this->data.'"),"01")';
			break;
			case '1':
			$filename = "/tmp/kvartplata.csv";
			$this->sql='SELECT cast(t1.`utszn_id`  as char) ,cast(t1.`oshad_id`  as char),cast(t1.`fio`  as char),cast(t1.`address_id`  as char),replace(cast(t1.`nachisleno`  as
			char),".",","),replace(cast(t1.`dolg`  as char),".",",") FROM OSBB.SUBS_KVARTPLATA as t1 WHERE t1.osmd_id = '.$this->osmd_id.' and t1.`data` =CONCAT(EXTRACT(YEAR_MONTH FROM "'.$this->data.'"),"01")';
			break;
			case '2':
			$filename = "/tmp/otoplenie.csv";
			$this->sql='SELECT cast(t1.`utszn_id`  as char) ,cast(t1.`oshad_id`  as char),cast(t1.`fio`  as char),cast(t1.`address_id`  as char),replace(cast(t1.`nachisleno`  as
			char),".",","),replace(cast(t1.`dolg`  as char),".",",") FROM OSBB.SUBS_TEPLO as t1 WHERE t1.osmd_id = '.$this->osmd_id.' and t1.`data` =CONCAT(EXTRACT(YEAR_MONTH FROM "'.$this->data.'"),"01")';
			break;
			case '3':
			$filename = "/tmp/voda.csv";
			$this->sql='SELECT cast(t1.`utszn_id`  as char) ,cast(t1.`oshad_id`  as char),cast(t1.`fio`  as char),cast(t1.`address_id`  as char),replace(cast(t1.`nachisleno`  as
			char),".",","),replace(cast(t1.`dolg`  as char),".",",") FROM OSBB.SUBS_VODA as t1 WHERE t1.osmd_id = '.$this->osmd_id.' and  t1.`data` =CONCAT(EXTRACT(YEAR_MONTH FROM "'.$this->data.'"),"01")';
			break;						
			case '4':
			$filename = "/tmp/energy.csv";
			$this->sql='SELECT cast(t1.`utszn_id`  as char) ,cast(t1.`oshad_id`  as char),cast(t1.`fio`  as char),cast(t1.`address_id`  as char),replace(cast(t1.`nachisleno`  as
			char),".",","),replace(cast(t1.`dolg`  as char),".",",") FROM OSBB.SUBS_ENERGY as t1 WHERE t1.osmd_id = '.$this->osmd_id.' and  t1.`data` =CONCAT(EXTRACT(YEAR_MONTH FROM "'.$this->data.'"),"01")';
			break;
			case '5':
			$filename = "/tmp/tbo.csv";
			$this->sql='SELECT cast(t1.`utszn_id`  as char) ,cast(t1.`oshad_id`  as char),cast(t1.`fio`  as char),cast(t1.`address_id`  as char),replace(cast(t1.`nachisleno`  as
			char),".",","),replace(cast(t1.`dolg`  as char),".",",") FROM OSBB.SUBS_TBO as t1 WHERE t1.osmd_id = '.$this->osmd_id.' and  t1.`data` =CONCAT(EXTRACT(YEAR_MONTH FROM "'.$this->data.'"),"01")';
			break;		
			case '11':
			$filename = "/tmp/kvartplata.csv";
			$this->sql='SELECT cast(t1.`utszn_id`  as char) ,cast(t1.`oshad_id`  as char),cast(t1.`fio`  as char),cast(t1.`address_id`  as char),replace(cast(t1.`nachisleno`  as
			char),".",","),replace(cast(t1.`dolg`  as char),".",",") FROM OSBB.LG_KVARTPLATA as t1 WHERE t1.osmd_id = '.$this->osmd_id.' and t1.`data` =CONCAT(EXTRACT(YEAR_MONTH FROM "'.$this->data.'"),"01")';
			break;
			case '22':
			$filename = "/tmp/otoplenie.csv";
			$this->sql='SELECT cast(t1.`utszn_id`  as char) ,cast(t1.`oshad_id`  as char),cast(t1.`fio`  as char),cast(t1.`address_id`  as char),replace(cast(t1.`nachisleno`  as
			char),".",","),replace(cast(t1.`dolg`  as char),".",",") FROM OSBB.LG_TEPLO as t1 WHERE t1.osmd_id = '.$this->osmd_id.' and t1.`data` =CONCAT(EXTRACT(YEAR_MONTH FROM "'.$this->data.'"),"01")';
			break;
			case '33':
			$filename = "/tmp/voda.csv";
			$this->sql='SELECT cast(t1.`utszn_id`  as char) ,cast(t1.`oshad_id`  as char),cast(t1.`fio`  as char),cast(t1.`address_id`  as char),replace(cast(t1.`nachisleno`  as
			char),".",","),replace(cast(t1.`dolg`  as char),".",",") FROM OSBB.LG_VODA as t1 WHERE t1.osmd_id = '.$this->osmd_id.' and  t1.`data` =CONCAT(EXTRACT(YEAR_MONTH FROM "'.$this->data.'"),"01")';
			break;						
			case '44':
			$filename = "/tmp/energy.csv";
			$this->sql='SELECT cast(t1.`utszn_id`  as char) ,cast(t1.`oshad_id`  as char),cast(t1.`fio`  as char),cast(t1.`address_id`  as char),replace(cast(t1.`nachisleno`  as
			char),".",","),replace(cast(t1.`dolg`  as char),".",",") FROM OSBB.LG_ENERGY as t1 WHERE t1.osmd_id = '.$this->osmd_id.' and  t1.`data` =CONCAT(EXTRACT(YEAR_MONTH FROM "'.$this->data.'"),"01")';
			break;
			case '55':
			$filename = "/tmp/tbo.csv";
			$this->sql='SELECT cast(t1.`utszn_id`  as char) ,cast(t1.`oshad_id`  as char),cast(t1.`fio`  as char),cast(t1.`address_id`  as char),replace(cast(t1.`nachisleno`  as
			char),".",","),replace(cast(t1.`dolg`  as char),".",",") FROM OSBB.LG_TBO as t1 WHERE t1.osmd_id = '.$this->osmd_id.' and  t1.`data` =CONCAT(EXTRACT(YEAR_MONTH FROM "'.$this->data.'"),"01")';
			break;								
			case 'privatbank':
			...
			$this->sql_trancate='CALL OSBB.privatbank_insert_newdata()';
			$this->sql='SELECT "LS", "FIO", "STREET", "BULDING", "FLAT", "SERVICECOD", "SUM" UNION ALL SELECT OSBB.DBF_PRIVATBANK.LS,OSBB.DBF_PRIVATBANK.FIO,OSBB.DBF_PRIVATBANK.STREET,
			OSBB.DBF_PRIVATBANK.BUILDING, OSBB.DBF_PRIVATBANK.FLAT, OSBB.DBF_PRIVATBANK.SERVICECOD, OSBB.DBF_PRIVATBANK.SUM FROM OSBB.DBF_PRIVATBANK';
			...
			$mail->Host = 'mail.yuzhny.com'; // SMTP сервер
