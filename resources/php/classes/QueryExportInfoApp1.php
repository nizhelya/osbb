...
class QueryExportInfoApp
{
	...
	public function exportToEmail(stdClass $params)
	{
		...
		$this->sql_callback='SELECT @success';
		...
		if($this->results['success'] ) {
			    ...
			    $x=0;
			    while ($x++<$this->results['success']) {
			      $this->sql_callback_send='SELECT t1.`address_id`, t1.`address`,t1.`email`,t1.`href` FROM YISGRAND.INFO_EMAIL as t1 WHERE t1.`osmd_id` = '.$this->osmd_id.' and t1.`chek` = 0 limit 1'; 		  
			      $this->res_callback_send = $_db->query($this->sql_callback_send) or die('Connect Error >>>  ' . $_db->connect_errno . '  <<< ' . $_db->connect_error);
			      $row_email=mysqli_affected_rows($_db); 
			      if($row_email){
			      ...
			      З повагою, «ЮКІС»!' ; // тело письма в html формате
			      if(!$cloned->Send()) {
				  ...
				  $this->results['msg'] = "Ошибка при отправлении почты";
